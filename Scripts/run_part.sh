...
time . utils/parse_options.sh  # accept options.. you can run this run.sh with the
...
if [ $stage -le 0 ]; then
  ...
  time local/prepare_data.sh || exit 1

  time local/prepare_dict.sh || exit 1

  time utils/prepare_lang.sh data/local/dict_nosp \
    "<unk>" data/local/lang_nosp data/lang_nosp || exit 1

  time local/prepare_lm.sh || exit 1
...
if [ $stage -le 1 ]; then
  for set in test dev train; do
    dir=data/$set
    time steps/make_mfcc.sh --nj 20 --cmd "$train_cmd" $dir $dir/log $dir/data || exit 1
    time steps/compute_cmvn_stats.sh $dir $dir/log $dir/data || exit 1
  done
fi
...
if [ $stage -le 2 ]; then
  time utils/subset_data_dir.sh --shortest data/train 10000 data/train_10kshort || exit 1
  time utils/data/remove_dup_utts.sh 10 data/train_10kshort data/train_10kshort_nodup || exit 1
fi

# Train
if [ $stage -le 3 ]; then
  time steps/train_mono.sh --nj 20 --cmd "$train_cmd" \
    data/train_10kshort_nodup data/lang_nosp exp/mono0a || exit 1

  time steps/align_si.sh --nj $nj --cmd "$train_cmd" \
  ...
  time utils/mkgraph.sh data/lang_nosp_test exp/tri1 exp/tri1/graph_nosp || exit 1

  time steps/decode.sh --nj $decode_nj --cmd "$decode_cmd" \
    --num-threads 4 \
...
if [ $stage -le 4 ]; then
  ...
  time utils/mkgraph.sh data/lang_nosp_test exp/tri2 exp/tri2/graph_nosp || exit 1
...
if [ $stage -le 5 ]; then
  time steps/get_prons.sh --cmd "$train_cmd" data/train data/lang_nosp exp/tri2
  time utils/dict_dir_add_pronprobs.sh --max-normalize true \
  ...
  time utils/prepare_lang.sh data/local/dict "<unk>" data/local/lang data/lang
  ...
  time utils/mkgraph.sh data/lang_test exp/tri2 exp/tri2/graph || exit 1
